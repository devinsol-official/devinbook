...
export function EditAccountModal({ isOpen, onClose, onSuccess, account }: EditAccountModalProps) {
    ...
    useEffect(() => {
        if (account) {
            ...
            setAutoLog(account.autoLog || false)
            setDefaultItems(account.defaultItems || [
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true)
            await api.updateAccount(account.id, { name, type, isDefault, isFeatured, autoLog, defaultItems: type === "regular billing" ? defaultItems : undefined })
